...
public class SuperAdminDashboardBal {
	...
	public static HashMap<String, String> getRecoveryRate() {
		HashMap<String, String> map = new HashMap<>();
		try (Connection connection = Connect.getConnection()) {
			if (connection != null) {
				CallableStatement prepareCall = connection
						.prepareCall("{call get_total_recovery_rate()}");
	...
	public static HashMap<String, String> getParTen() {
		HashMap<String, String> map = new HashMap<>();
		try (Connection connection = Connect.getConnection()) {
			if (connection != null) {
				CallableStatement prepareCall = connection
						.prepareCall("{call get_total_parten()}");
				ResultSet rs = prepareCall.executeQuery();
				while (rs.next()) {
					map.put("defaulters", rs.getInt(1) + "");
					map.put("parTen", Math.round(rs.getDouble(2)) + "");
				}
			}
		} catch (SQLException e) {
			logger.error("", e);
