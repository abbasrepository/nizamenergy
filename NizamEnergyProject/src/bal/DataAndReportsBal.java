...
public class DataAndReportsBal {
	...
	public static ArrayList<HashMap<String, String>> getList(int foId) {
		ArrayList<HashMap<String, String>> list = new ArrayList<>();
		try (Connection connection = Connect.getConnection()) {
			ResultSet resultSet = null;
			CallableStatement prepareCall = connection
					.prepareCall("{CALL get_customer_list_fo_wise(?)}");
			...
			while (resultSet.next()) {
				...
				map.put("customerName", resultSet.getString("customer_name"));
				map.put("customerPhone", resultSet.getString("customer_phone"));
				map.put("installment",resultSet.getInt("installment_amount_month") + "");
				map.put("NdName", resultSet.getString("salesman_name"));
				map.put("NdPhone",resultSet.getString("salesman_primary_phone"));
				map.put("duedate", resultSet.getDate("due_date") + "");
				list.add(map);
			}
		} catch (SQLException e) {
			logger.error("", e);
	...
	public static ArrayList<HashMap<String, String>> getDoRecovery() {
		ArrayList<HashMap<String, String>> list = new ArrayList<>();
		try (Connection connection = Connect.getConnection()) {
			ResultSet resultSet = null;
			CallableStatement prepareCall = connection
					.prepareCall("{CALL get_do_current_recovery_rate()}");
			resultSet = prepareCall.executeQuery();
			while (resultSet.next()) {
				HashMap<String, String> map = new HashMap<>();
				map.put("doName", resultSet.getString("user_name") + "");
	...
	public static ArrayList<HashMap<String, String>> getFoRecovery() {
		ArrayList<HashMap<String, String>> list = new ArrayList<>();
		try (Connection connection = Connect.getConnection()) {
			ResultSet resultSet = null;
			CallableStatement prepareCall = connection
					.prepareCall("{CALL get_fo_wise_current_recovery_rate()}");
			resultSet = prepareCall.executeQuery();
			while (resultSet.next()) {
				HashMap<String, String> map = new HashMap<>();
				map.put("district", resultSet.getString("district_name") + "");
	...
	public static void main(String arg[]) {
		System.err.println(getDoRecovery());
	}
