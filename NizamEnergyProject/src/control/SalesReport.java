	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			if (action.equals("Generate Customer Rating Report")) {
				...
				ArrayList<HashMap<String, String>> list1 = DataAndReportsBal
						.getRecoveryReport(doId);
			...
			} else if (action.equals("Download CustomerList")) {
				int foId = Integer.parseInt(request.getParameter("foId"));
				HSSFWorkbook workbook = new HSSFWorkbook();
				HSSFSheet sheet = workbook.createSheet("FO RecoveryReport");
				...
				header.createCell(4).setCellValue("ND Name");
				header.createCell(5).setCellValue("ND Number");
				...
				for (int i = 0; i < list.size(); i++) {
					...
					dataRow.createCell(3).setCellValue(
							list.get(i).get("installment") + "");
					dataRow.createCell(4).setCellValue(
							list.get(i).get("NdName") + "");
					dataRow.createCell(5).setCellValue(
							list.get(i).get("NdPhone") + "");
				}
				response.setContentType("application/vnd.ms-excel");
				response.setHeader("Content-Disposition",
						"attachment; filename=CustomerList:report.xls");
				workbook.write(response.getOutputStream());
				response.getOutputStream().flush();
			}
		} catch (Exception e) {
