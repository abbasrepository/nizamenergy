...
function getCustomerAcceptedAppliances(customerId) {
	$
			.ajax({
				...
				dataType : 'json',
				data : {
					action : 'getCustomerAcceptedAppliances',
					customerId : customerId
				},
				success : function(json) {
					if (customerId != null && json != null) {
						$('#getAcceptedAppliances').empty();
						if (json.data.length > 0) {
							$
									.each(
											json.data,
											function(e) {
												...
												var getStatus;
												var getTransferStatus;
												var applianceChargingStatus;
												var getApplianceStatus;
												var loanBookView = null;

												if (json.data[e].status == '0') {
													getStatus = '<span class="label" style="background-color: #bdc3c7; color: white; font-weight: bold;">Applied</span>';
												} else if (json.data[e].status == '1') {
													getStatus = '<span class="label" style="background-color: #2980b9; color: white; font-weight: bold;">Accepted</span>';
												} else if (json.data[e].status == '3') {
													getStatus = '<span class="label" style="background-color: #2980b9; color: white; font-weight: bold;">Not Interested</span>';
												} else if (json.data[e].status == '4') {
													getStatus = '<span class="label" style="background-color: #2980b9; color: white; font-weight: bold;">Under CC Varification</span>';
												} else if (json.data[e].status == '6'
														|| json.data[e].status == '7'
														|| json.data[e].status == '8') {
													getStatus = '<span class="label" style="background-color: #2ecc71; color: white; font-weight: bold;">Verified</span>';
												} else {
													getStatus = 'N/A';
												}

												if (json.data[e].applianceStatus != null) {
													if (json.data[e].applianceStatus == '0') {
														getTransferStatus = '<span class="label" style="background-color: #bdc3c7; color: white; font-weight: bold;">Pending</span>';
													} else if (json.data[e].applianceStatus == '1') {
														...
														getTransferStatus = '<span class="label" style="background-color: #1abc9c; color: white; font-weight: bold;">Awaiting Downpayment</span>';
													} else if (json.data[e].applianceStatus == '6') {
														getTransferStatus = '<span class="label" style="background-color: green; color: white; font-weight: bold;">Installed</span>';
													} else {
														getTransferStatus = '<span class="label" style="background-color: #bdc3c7; color: white; font-weight: bold;">Pending</span>';
													}
												} else {
													getTransferStatus = 'N/A';
												}

												if (json.data[e].appliance_status == '1') {
													applianceChargingStatus = '<span class="label" style="background-color: #16a085; color: white; font-weight: bold;">Active</span>';
												} else {
													applianceChargingStatus = '<span class="label" style="background-color: Red; color: white; font-weight: bold;">Inactive</span>';
												}

												if (json.data[e].status_get == 'maintained') {
													getApplianceStatus = '<span class="label" style="background-color: blue; color: white; font-weight: bold;">Maintained</span>';
												} else if (json.data[e].status_get == 'owned') {
													getApplianceStatus = '<span class="label" style="background-color: #7f8c8d; color: black; font-weight: bold;">Owned</span>';
												} else if (json.data[e].status_get == 'defaulter') {
													getApplianceStatus = '<span class="label" style="background-color: #34495e; color: white; font-weight: bold;">Defaulter</span>';
												} else {
													getApplianceStatus = 'N/A';
												}

												if (json.data[e].status_get == 'maintained'
														|| json.data[e].status_get == 'owned'
														|| json.data[e].status_get == 'defaulter') {
													loanBookView = '<a href="SuperAdminLoanBookController?appliace_key='
															+ json.data[e].appliance_id
															+ '"><span class="label" style="background-color: #34495e; color: white; font-weight: bold; width: 100%">View</span></a>';
												} else {
													loanBookView = 'N/A';
												}

												var tds = '<tr>'
														+ '<td><a href="ViewServlet?click=viewAppliance&id='
														+ json.data[e].appliance_id
														+ '">'
														+ json.data[e].appliance_name
														+ '</a></td>'
														+ '<td><a href="DistrictOfficer?do_id='
														+ json.data[e].user_id
														+ '">'
														+ json.data[e].user_name
														+ '</td>'
														+ '<td><a href="fieldOfficerProfile.jsp?fo_id='
														+ json.data[e].fo_id
														+ '">'
														+ json.data[e].fo_name
														+ '</td>'
														+ '<td><a href="NizamDostViewController?click=ok&salesman_id='
														+ json.data[e].salesman_id
														+ '">'
														+ json.data[e].salesman_name
														+ '</td>'
														+ '<td>'
														+ getStatus
														+ '</td>'
